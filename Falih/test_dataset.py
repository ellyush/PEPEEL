...
class TestDataset(unittest.TestCase):
    ...
    def test_upload_dataset_size(self):
        self.assertEqual(upload_dataset("csv", file_size_mb=1), "File terlalu besar")
    def test_upload_dataset_size(self):
        self.assertEqual(upload_dataset("csv", file_size_mb=50), "File terlalu besar")
    def test_upload_dataset_size(self):
        self.assertEqual(upload_dataset("csv", file_size_mb=250), "File terlalu besar")

    # TC-37 upload dataset dari link public
    def test_upload_dataset_public_link(self):
        self.assertEqual(upload_dataset("csv", link="http://example.com", is_private=False), "Upload dari link sukses")

    # TC-38 upload dataset dari link private
    def test_upload_dataset_private_link(self):
        self.assertEqual(upload_dataset("csv", link="http://example.com", is_private=True), "Link tidak bisa diakses")
